...
class Country(models.Model):
    ...
    numbering_format = models.CharField(
        "Numbering Format",
        max_length=10,
        choices=NUMBERING_FORMAT_CHOICES,
        default="1,000.00",
    )
    ...
    def save(self, *args, **kwargs):
        """
        Auto-generate a unique slug based on the country name.

        Uses self.__class__ to avoid circular imports and ensures that
        if a slug already exists, it appends "-1", "-2", etc.
        """
        if not self.slug and self.name:
            ...
            ModelClass = self.__class__  # avoids circular import
